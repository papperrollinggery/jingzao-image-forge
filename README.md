...
**Jingzao Image Forge (镜造 Image Forge)** is a Codex Skill for structured image prompt engineering. It turns image briefs, reference-image observations, and local edit requests into a maintainable `visual_generation_spec`, then compiles that specification for OpenAI GPT Image 2, FLUX, Midjourney, or a generic image generator.
...
Current local baseline: **21 regression tests** covering validation, material-schema integrity, local-edit constraints, provider compilation, knowledge-anchor preservation, adaptive hybrid grounding, artifact budgets, and cross-platform prompt survival.
