...
**v1.9.0:** Adds the optional [Soft Editorial Character family](references/soft-editorial-style.md), three lighting/wardrobe branches and a [body-profile form](references/body-profile.md) for every person-style distillation. The new capsule remains **draft / partially verified**: one returned image has deviations and two tests were output-blocked. Installation and release do not imply visual acceptance.
...
## Production Coverage and Per-Frame Handoff

Use the optional [production coverage workflow](references/production-coverage.md) when images must support an approved shot list. Each shot names its required image anchors or explains why video-only motion is sufficient. Reverse views preserve world geometry while changing camera-relative placement; changed prop states receive separate image requirements when needed.
...
Each frame contains a complete existing image spec, so its prompt, exact text, references, and preflight remain isolated. Legacy `styleboard` compilation returns one planning package, not automatically split image calls. The new compiler does not generate images or video, infer missing story beats, or certify visual continuity. DIR or the upstream workflow retains story, timing, video prompts, and the project ledger. Existing single-image and exploratory-sheet routes are unchanged.

## Quality Recovery Without Style Loss

The [quality workflow](references/quality-controls.md) now separates source contamination, material response, optics and export defects before choosing a retry. Cleanup suggestions preserve the requested medium, intended wet/gloss response, identity edges and exact text. Fixed universal denoise suffixes are not injected; the ordinary clean base and semantic budgets are unchanged. A cleaner candidate must still pass action, object-count, camera, scale and continuity checks. A texture-free layout proxy is an optional fallback after demonstrated reference carryover, not a new default pipeline or pixel-denoising guarantee.
...
## Reusable Fantasy and Summer Style Packages

The Changsheng wardrobe revision now derives silhouette, palette, pattern coverage, material behavior and dressing from the character and situation. A new 149-sample native-resolution film survey and six reviewed image tests cover plain, pale, rich, layered, single-visible-garment and story-close-up cases. See [the wardrobe system](references/changsheng-wardrobe-system.md). Historical v1.7.0 prompts retain an immutable capsule fixture and their original hashes.


- [Changsheng / 长生·华构人间](references/changsheng-style.md): inhabited grand architecture, role-based hair/makeup and costume, multi-angle narrative frames, plus separately identified original magic-combat tests.
- [Azure Summer / 晴海·角色写真](references/azure-summer-style.md): airy daylight, canopy shade, sunset and close interaction branches; portrait, widescreen and ultrawide restaging; explicit ownership when combined with grand architecture.
- [Body and garment fit](references/body-and-garment-fit.md): preserve or specify requested adult proportions independently of clothing coverage and style; use appropriate tailoring and fabric support instead of silently flattening or exaggerating bodies.

Invoke the named style, then supply the subject, event, camera and aspect ratio. These additions reuse the existing compiler and are opt-in. Raw reference and new generated test pixels are not in the release; local visual-review records state exact limitations, including incomplete magic contact and neckline drift during one edit.
