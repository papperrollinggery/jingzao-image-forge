...
- For a release install or sync from a resolved commit or tag, run `scripts/verify_install.py <source-repo> <installed-dir> --ref <commit-or-tag>`. A match proves only that the installed payload matches the resolved source commit at the tracked-file and symlink level; empty directories carry no Git payload and are outside this check. It does not prove automatic adoption in an already-open task. Development working-tree copies are not release-parity evidence; validate them with regression tests until committed.
