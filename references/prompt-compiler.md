...
The maintained specification may stay detailed, but the disposable compiled prompt is normalized per provider without deleting explicit source fields. When the final prompt exceeds its review target, the compiler reports `review_required`; the author removes duplicated or unnecessary clauses from the source specification and recompiles. Review approval changes execution status only and never produces a hidden shorter prompt. Actual references, exact text, visible events, relationships, subject identity/action, camera/viewer geometry, color/film behavior, material/render transport, edits, preserve/change invariants, and a causal effect's owner/contact/cost/response/residue chain remain intact.
...
The neutral template uses `canvas.aspect_ratio: auto`, empty camera/position fields, and `render.artifact_budget: auto`. These values emit no visual instruction. A template with no semantic generation content is `blocked` with reason `empty_prompt`, even if provider flags exist. Explicit non-empty professional fields always compile; progressive detail governs what the author adds, not whether the compiler silently discards requested controls.
