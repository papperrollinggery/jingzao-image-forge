...
For `styleboard`, `compile_prompt.py` compiles the global visual master, reference assignments, continuity locks, frame cards, and strategy as one planning package. `sheet_direct` requests one equal-cell board and carries grid geometry, reading order, panel count, and continuity locks. `independent_frames` and `hybrid` describe execution intent; this compiler does not automatically separate their frame prose or attachments. For independently executable native-ratio frame packages, author one complete image spec per required frame and use `scripts/compile_production.py` with [production-coverage.md](production-coverage.md). Do not represent any strategy as universally superior.
